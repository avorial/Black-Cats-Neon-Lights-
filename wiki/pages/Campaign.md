---
category: game
type: game
name: Campaign
summary: ""
tags:
  - game
visibility: gm
approvalStatus: unapproved
knownToPlayers: false
keyLinks: []
aliases: []
lastEditedBy: AI via MCP
---

# Campaign

## Overview

King in Yellow downtime archive for the Black Cats & Neon Lights chronicle. This page indexes the cleaned downtime import and preserves the source as a readable retrospective reference.

## Source

- Source requested: `006 - Players Downtimes.zip`
- Working source located in Downloads: `Downtime_Markdown.zip`
- Related uploaded archive present: `The Yellow King ~ Black Cats & Neon Lights.zip`

## Player Characters

- [[Ariel Fontaine]]
- [[Dorian Van Der Lynden]]
- [[Herman]]
- [[Molly]]
- [[Newland Van Der Lynden]]
- [[Regina Wentworth]]

## Core References

- [[Timeline]]
- [[The King in Yellow (Performances)]]
- [[Unidentified Forces by Albert de Rochas]]
- [[From the Sun Comes Shadows]]
- [[The Medical Case of Maribel Fontaine]]

## Major Threads

- [[The Red Knight]]
- [[Carcosan Absinthe]]
- [[Crystal Shards from the Ibroix]]
- [[Kaladi Bedur]]
- [[The Ibroix]]
- [[The Warton Players]]
- [[Carcosa]]

## Import Notes

- Dates were normalized to `YYYY-MM-DD` where filenames or headings gave dates.
- Undated entries use inferred dates where source order and surrounding context made a reasonable placement possible.
- Inferred dates are marked with `(inferred)`.
- Canonical normalized names: [[Rachel Weissman]], [[Chloe]], [[Phillip Du Pol]], and [[Doctor Sasha Kersiki]].
- The archive preserves ambiguity around identities, doubles, cult affiliations, and supernatural mechanisms.